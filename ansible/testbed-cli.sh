...
while i < len(lines):
    ...
    if m:
        host_indent = m.group(1)
        # Derive the property indentation from the block's own first property
        # line (inventories vary between 2- and 4-space steps); fall back to
        # host indent + 2 for an empty block.
        prop_indent = host_indent + '  '
        j = i + 1
        while j < len(lines) and not lines[j].strip():
            j += 1
        if j < len(lines):
            pm = re.match(r'^(\s+)\S', lines[j])
            if pm and len(pm.group(1)) > len(host_indent):
                prop_indent = pm.group(1)
        j = i + 1
...
if updated:
    ...
    sys.stderr.write("WARNING: host {} not found in {} (YAML mapping-style inventories only); "
                     "add 'vm_type: <type>' to the host manually\n".format(dut, inv_file))
PYEOF
  done
}
...
function add_topo
{
  ...
  read_file ${testbed_name}
  # Persist vm_type only when the operator passed -k explicitly; otherwise a
  # legacy invocation without -k would overwrite a previously stored value
  # with the default.
  if [[ -n "$vm_type_explicit" ]]; then
