...
def test_portchannel_member_remove_and_readd(
    ...
    3. Remove the member via GCU; assert: PORTCHANNEL_MEMBER row gone, teamd no longer
       lists the port, remaining members still selected, BGP still established, prefix
       still present, traffic still forwarded on the remaining members and none on the
       removed port.
    4. Add the member back via GCU; assert: row restored, port selected again, BGP
    ...
    remaining = members[:-1]
    logger.info(
        "scenario=%s: GCU member cycle on %s towards %s: remove/re-add %s, keep %s",
        pc_scenario["id"], portchannel, neighbor_ctx["neighbor_name"], cycled, remaining,
    )
    ...
    try:
        with allure.step(f"Remove member {cycled} from {portchannel} via GCU and validate"):
            ...
            pytest_assert(
                verify_prefix_present(duthost, dst_asic, target, neighbor_ctx, should_exist=True),
                f"Prefix {target['prefix']} via {neighbor_ctx['neighbor_name']} must survive removing one LAG member",
            )
