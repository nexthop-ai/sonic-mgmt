...
def init_dhcpmon_counters(duthost, is_v6=False):
    ...
    pytest_assert(
        "Clear DHCP{} relay counter done".format("v6" if is_v6 else "v4") == command_output["stdout"],
...
def query_dhcpmon_counter_result(duthost, query_key, is_v6=False):
    ...
    """
    counters_query_string = 'sonic-db-cli COUNTERS_DB hgetall "DHCP{}_COUNTER_TABLE:{}"'.format(
        "V6" if is_v6 else "V4", query_key
    )
    shell_result = json.loads(duthost.shell(counters_query_string)["stdout"].replace('"', "").replace("'", '"'))
    return {
        rx_or_tx: {dhcp_type: int(counter_value) for dhcp_type, counter_value in counters.items()}
...
def compare_dhcp_counters_with_warning(
    actual_counter, expected_counter, warning_msg, error_in_percentage=0.0, is_v6=False
):
    compare_result = compare_dhcp_counters(actual_counter, expected_counter, error_in_percentage, is_v6)
...
def compare_dhcp_counters(actual_counter, expected_counter, error_in_percentage=0.0, is_v6=False):
    ...
    for dir in SUPPORTED_DIR:
        for dhcp_type in SUPPORTED_DHCPV6_TYPE if is_v6 else SUPPORTED_DHCPV4_TYPE:
            ...
            pytest_assert(
                abs(actual_value - expected_value) <= int(expected_value * error_in_percentage / 100),
                "DHCP relay counter {} {} {} is not equal to expected value {} within error {}%".format(
                    dir, dhcp_type, actual_value, expected_value, error_in_percentage
                ),
            )


def validate_dhcpmon_counters(
    dhcp_relay, duthost, expected_uplink_counter, expected_downlink_counter, error_in_percentage=0.0, is_v6=False
):
    """Validate the dhcpmon counters against the expected counters."""
    logger.info(
        "Expected uplink counters: {}, expected downlink counters: {}, error in percentage: {}%".format(
            expected_uplink_counter, expected_downlink_counter, error_in_percentage
        )
    )
    downlink_vlan_iface = dhcp_relay["downlink_iface"]["name"]
    ...
    for portchannel_name in uplink_portchannels_or_interfaces:
        if portchannel_name in portchannels.keys():
            ...
            portchannel_counters = query_and_sum_dhcpmon_counters(
                duthost, downlink_vlan_iface, [portchannel_name], is_v6
            )
            members_counters = query_and_sum_dhcpmon_counters(
                duthost, downlink_vlan_iface, portchannels[portchannel_name]["members"], is_v6
            )
    ...
    if client_iface != downlink_vlan_iface:
        compare_dhcp_counters_with_warning(
    ...
    downlink_counter = vlan_interface_counter if client_iface == downlink_vlan_iface else client_interface_counter
    compare_dhcp_counters_with_warning(
        downlink_counter,
        expected_downlink_counter,
        compare_warning_msg.format(
            downlink_vlan_iface if client_iface == downlink_vlan_iface else client_iface,
            "expected_downlink_counter",
            duthost.hostname,
        ),
        error_in_percentage,
...
def calculate_counters_per_pkts(pkts, is_v6=False):
    ...
    for pkt in pkts:
        if hasattr(pkt, "ifindex") and pkt.haslayer(scapy.DHCP6 if is_v6 else scapy.DHCP):
            counter = all_counters.setdefault(pkt.ifindex, {"RX": {}, "TX": {}})
            if is_v6:
                ...
                for data in pkt[scapy.DHCP].options:
                    if isinstance(data, tuple):
                        opt, val = data
                        if opt == "message-type":
                            message_type_int = val
            message_type_str = (
                (SUPPORTED_DHCPV6_TYPE if is_v6 else SUPPORTED_DHCPV4_TYPE)[message_type_int - 1]
                if message_type_int is not None and message_type_int > 0
                else "Unknown"
            )
...
def validate_counters_and_pkts_consistency(
    dhcp_relay, duthost, pkts, interface_name_index_mapping, error_in_percentage=0.0, is_v6=False
):
    ...
    for portchannel_name in uplink_portchannels_or_interfaces:
        if portchannel_name in portchannels.keys():
            ...
            portchannel_counter_from_pkts = all_pkt_counters.get(
                interface_name_index_mapping[portchannel_name], {"RX": {}, "TX": {}}
            )

            members_counter_from_pkts = {"RX": {}, "TX": {}}
            ...
            for member in portchannels[portchannel_name]["members"]:
                merge_counters(
                    ...
                    all_pkt_counters.get(interface_name_index_mapping[member], {"RX": {}, "TX": {}}),
                    is_v6,
                )
    ...
    vlan_interface_counter_from_pkts = all_pkt_counters.get(
        interface_name_index_mapping[downlink_vlan_iface], {"RX": {}, "TX": {}}
    )

    # calculate the sum of uplink portchannels interfaces counters from pkts
    uplink_portchannels_interfaces_counter_from_pkts = {"RX": {}, "TX": {}}
    ...
    uplink_interface_counter_from_pkts = {"RX": {}, "TX": {}}
    ...
    compare_dhcp_counters_with_warning(
        vlan_interface_counter,
        vlan_interface_counter_from_pkts,
    ...
    compare_dhcp_counters_with_warning(
        ...
        uplink_portchannels_interfaces_counter_from_pkts,
        compare_warning_msg.format(
            uplink_portchannels_or_interfaces, str(uplink_portchannels_or_interfaces) + " from pkts", duthost.hostname
        ),
        error_in_percentage,
        is_v6,
    )
    ...
    for vlan_member in vlan_members:
        ...
        vlan_member_counter_from_pkts = all_pkt_counters.get(
            interface_name_index_mapping[vlan_member], {"RX": {}, "TX": {}}
        )
        compare_dhcp_counters_with_warning(
            vlan_member_counter,
            vlan_member_counter_from_pkts,
