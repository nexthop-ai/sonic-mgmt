...
def _get_transport_src_ports(duthost):
    """Return {oid_key: src_port} for every TAM transport in ASIC_DB that has one.

    A stale/orphaned TAM_TRANSPORT object (e.g. left over from incomplete
    cleanup of a prior test) may lack SAI_TAM_TRANSPORT_ATTR_SRC_PORT. Such
    entries aren't part of the session under test, so they're excluded here
    rather than corrupting the pre/post-reboot comparison with a None value.
    """
    keys = redis_get_keys(duthost, "ASIC_DB",
                          "ASIC_STATE:SAI_OBJECT_TYPE_TAM_TRANSPORT:oid:*") or []
    src_ports = {key: redis_hgetall_all_asics(duthost, "ASIC_DB", key)
                 .get("SAI_TAM_TRANSPORT_ATTR_SRC_PORT") for key in keys}
    return {key: port for key, port in src_ports.items() if port is not None}
