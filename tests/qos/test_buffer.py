...
def test_change_speed_cable(duthosts, rand_one_dut_hostname, conn_graph_facts,  # noqa: F811
    ...
    if check_qos_db_fv_reference_with_table(duthost) is True:
        ...
        profile = "BUFFER_PROFILE_TABLE:" + \
            duthost.shell(
                'redis-cli hget "BUFFER_PG_TABLE:{}:3-4" profile'.format(port_to_test))['stdout']
    detect_default_mtu(duthost, port_to_test)

    original_pg_size = int(duthost.shell(
        'redis-cli hget "{}" size'.format(profile))['stdout'])
    original_pool_size = int(duthost.shell(
        'redis-cli hget BUFFER_POOL_TABLE:ingress_lossless_pool size')['stdout'])
    if DEFAULT_OVER_SUBSCRIBE_RATIO:
        original_pg_xoff = int(duthost.shell(
            'redis-cli hget "{}" xoff'.format(profile))['stdout'])
        original_shp_size = int(duthost.shell(
            'redis-cli hget BUFFER_POOL_TABLE:ingress_lossless_pool xoff')['stdout'])
    else:
    ...
    if mtu_to_test == DEFAULT_MTU:
        if speed_to_test == original_speed and cable_len_to_test == original_cable_len:
            pytest.skip(
                'Speed, MTU and cable length matches the default value, nothing to test, skip')
        expected_profile = make_expected_profile_name(
            speed_to_test, cable_len_to_test)
        if duthost.shell('redis-cli hget BUFFER_PROFILE_TABLE:{}'.format(expected_profile))['stdout']:
            pytest.skip(
                'The buffer profile has existed, most of the checks can not be performed, skip')

    try:
        if not speed_to_test == original_speed:
            logging.info("Changing port's speed to {}".format(speed_to_test))
            duthost.shell('config interface speed {} {}'.format(
                port_to_test, speed_to_test))
        if not mtu_to_test == DEFAULT_MTU:
            logging.info("Changing port's mtu to {}".format(mtu_to_test))
            duthost.shell('config interface mtu {} {}'.format(
                port_to_test, mtu_to_test))
        if not cable_len_to_test == original_cable_len:
            logging.info(
                "Changing port's cable length to {}".format(cable_len_to_test))
            duthost.shell(
                'config interface cable-length {} {}'.format(port_to_test, cable_len_to_test))
        ...
        if mtu_to_test != DEFAULT_MTU:
            expected_profile = make_expected_profile_name(
                speed_to_test, cable_len_to_test, other_factors=['mtu{}'.format(mtu_to_test)])
        ...
        logging.info(
            '[Speed and/or cable-len and/or MTU updated] Checking whether new profile {} has been created '
            'and pfc_enable has been updated'.format(expected_profile))
        ...
        logging.info('SAI OID for newly created profile {} ingress lossless pool {}'.format(
            profile_oid, pool_oid))

        # Check whether profile exist
        pg_size = int(duthost.shell(
            'redis-cli hget "BUFFER_PROFILE_TABLE:{}" size'.format(expected_profile))['stdout'])
        pg_xoff = int(duthost.shell('redis-cli hget "BUFFER_PROFILE_TABLE:{}" xoff'.format(
            expected_profile))['stdout']) if DEFAULT_OVER_SUBSCRIBE_RATIO else None
        ...
        logging.info(
            '[Remove all lossless PGs] Checking pool size and pfc_enable')
        duthost.shell(
        ...
        if check_profile_removed:
            logging.info('[Remove dynamic profile on PG removed] Checking whether the profile {} is removed '
                         'on receiving all lossless PG removed'.format(expected_profile))
            check_lossless_profile_removed(
                duthost, expected_profile, profile_oid)

            # Re-add another lossless priority
            logging.info(
                'Re-add a lossless_pg and check pool size and pfc_enable')
            duthost.shell(
                'config interface buffer priority-group lossless add {} 6'.format(port_to_test))
            ...
            profile_oid, _ = check_buffer_profile_details(
                duthost, initial_asic_db_profiles, expected_profile, None, pool_oid, port_to_test)

            if cable_len_to_test != original_cable_len:
                logging.info(
                    '[Revert the cable length to the default value] Checking whether the profile is updated')
                duthost.shell(
                    'config interface cable-length {} {}'.format(port_to_test, original_cable_len))

            if mtu_to_test != DEFAULT_MTU:
                logging.info(
                    '[Revert the mtu to the default value] Checking whether the profile is updated')
                duthost.shell('config interface mtu {} {}'.format(
                    port_to_test, DEFAULT_MTU))

            # Remove old profile on cable length change
            logging.info(
                '[Remove dynamic profile on cable length and/or MTU updated] '
                'Checking whether the old profile is removed')
            check_lossless_profile_removed(
                duthost, expected_profile, profile_oid)
            expected_profile = make_expected_profile_name(
                speed_to_test, original_cable_len)
            check_pg_profile(duthost, 'BUFFER_PG_TABLE:{}:6'.format(
                port_to_test), expected_profile)
            ...
            duthost.shell(
                'config interface buffer priority-group lossless remove {} 6'.format(port_to_test))
            ...
            if cable_len_to_test != original_cable_len:
                logging.info(
                    '[Update cable length without any lossless pg configured]')
                duthost.shell(
                    'config interface cable-length {} {}'.format(port_to_test, original_cable_len))
            if mtu_to_test != DEFAULT_MTU:
        ...
        expected_profile = make_expected_profile_name(
            original_speed, original_cable_len)
        check_pg_profile(
        ...
        duthost.shell('config interface buffer priority-group lossless remove {}'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config interface speed {} {}'.format(
            port_to_test, original_speed), module_ignore_errors=True)
        duthost.shell('config interface mtu {} {}'.format(
            port_to_test, DEFAULT_MTU), module_ignore_errors=True)
        duthost.shell('config interface cable-length {} {}'.format(port_to_test,
                      original_cable_len), module_ignore_errors=True)
        duthost.shell('config interface buffer priority-group lossless add {} 3-4'.format(
            port_to_test), module_ignore_errors=True)
        ensure_pool_size(duthost, 60, original_pool_size,
                         original_shp_size, None)
...
def test_headroom_override(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test,     # noqa F811
    ...
    original_pg_size = duthost.shell(
        'redis-cli hget "{}" size'.format(original_profile))['stdout']
    original_pool_size = duthost.shell(
        'redis-cli hget BUFFER_POOL_TABLE:ingress_lossless_pool size')['stdout']
    if DEFAULT_OVER_SUBSCRIBE_RATIO:
        original_shp_size = duthost.shell(
            'redis-cli hget BUFFER_POOL_TABLE:ingress_lossless_pool xoff')['stdout']
        original_pg_xoff = duthost.shell(
            'redis-cli hget "{}" xoff'.format(original_profile))['stdout']
    else:
        original_shp_size = None
        original_pg_xoff = None
    ...
    try:
        ...
        if not param:
            pytest.skip(
                'Headroom override test skipped due to no parameters for "add" command provided')
        else:
            cli_str, new_size, new_xoff = _parse_buffer_profile_params(
        ...
        logging.info(
            "[Test: headroom override on lossless PG 3-4] Apply the profile on the PG and check pool size")
        duthost.shell(
            'config interface buffer priority-group lossless set {} 3-4 headroom-override'.format(port_to_test))

        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), 'headroom-override')
        check_pfc_enable(duthost, port_to_test, '3,4')
        profile_oid, pool_oid = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, "headroom-override", None, None, port_to_test)
        ...
        logging.info(
            "[Test: headroom override on more lossless PGs 6] Apply the profile on the PG and check pool size")
        duthost.shell(
            'config interface buffer priority-group lossless add {} 6 headroom-override'.format(port_to_test))

        check_pg_profile(duthost, 'BUFFER_PG_TABLE:{}:6'.format(
            port_to_test), 'headroom-override')
        check_pfc_enable(duthost, port_to_test, '3,4,6')
        profile_oid, _ = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, "headroom-override", profile_oid, pool_oid, port_to_test)
        ...
        if not param:
            pytest.skip(
                'Headroom override test skipped due to no parameters for "set" command provided')
        else:
            cli_str, new_size, new_xoff = _parse_buffer_profile_params(
                param, "set", "headroom-override")

        logging.info(
            "[Test: update headroom-override profile] Update the profile and check pool size: {}".format(cli_str))
        duthost.shell(cli_str)
        ...
        logging.info(
            "[Test: static headroom being referenced can not be removed]")
        duthost.shell(
            'config buffer profile remove headroom-override', module_ignore_errors=True)

        profile = duthost.shell(
            'redis-cli hgetall "BUFFER_PROFILE_TABLE:headroom-override"')['stdout']
        pytest_assert(
            profile, 'Headroom override profile has been removed when being referenced')
        logging.info("[Restore configuration]")
        duthost.shell(
            'config interface buffer priority-group lossless remove {}'.format(port_to_test))
        duthost.shell(
            'config interface buffer priority-group lossless add {} 3-4'.format(port_to_test))

        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), original_profile.split(':')[1])
        check_pfc_enable(duthost, port_to_test, '3,4')
        check_pool_size(duthost,
                        pool_oid,
                        pool_size=original_pool_size,
...
def check_buffer_profiles_for_shp(duthost, shp_enabled=True):
    def _check_buffer_profiles_for_shp(duthost, shp_enabled):
        buffer_profiles = duthost.shell(
            'redis-cli keys "BUFFER_PROFILE_TABLE:*"')['stdout'].split('\n')
        for profile_name in buffer_profiles:
            m = re.search(LOSSLESS_PROFILE_PATTERN, profile_name)
            if m:
                profile_obj = _compose_dict_from_cli(duthost.shell(
                    'redis-cli hgetall {}'.format(profile_name))['stdout'].split('\n'))
                if shp_enabled:
...
def test_shared_headroom_pool_configure(duthosts,
    ...
    how they affect the other buffer configurations
    ...
    if not TESTPARAM_SHARED_HEADROOM_POOL:
        pytest.skip(
            'Shared headroom pool test skipped due to no parameters defined')
    ...
    try:
        ...
        profile_oid, pool_oid = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, expected_profile, None, None, port_to_test)
        logging.info(
            'Got SAI OID of ingress lossless pool: {}'.format(pool_oid))
        ...
        logging.info(
            '[Test: check shared headroom pool size consistency between APPL_DB and ASIC_DB]')
        check_pool_size(duthost,
                        pool_oid,
                        pool_size=original_pool_size,
                        shp_size=original_shp_size,
        ...
        logging.info(
            '[Test: update over-subscribe-ratio to 4 and check sizes of buffer pool and shared headroom pool]')
        duthost.shell(
            'config buffer shared-headroom-pool over-subscribe-ratio 4')
        check_pool_size(duthost,
                        pool_oid,
                        pool_size=original_pool_size,
                        shp_size=original_shp_size,
        ...
        logging.info(
            '[Test: configure shared headroom pool size and check APPL_DB and ASIC_DB]')
        duthost.shell(
            'config buffer shared-headroom-pool size {}'.format(shp_size_to_test))
        check_pool_size(duthost,
                        pool_oid,
        ...
        logging.info(
            '[Test: remove the over subscribe ratio configuration while size is configured]')
        duthost.shell(
            'config buffer shared-headroom-pool over-subscribe-ratio 0')
        check_pool_size(duthost,
                        pool_oid,
        ...
        logging.info(
            '[Test: remove the size configuration while over subscribe ratio is configured]')
        duthost.shell(
            'config buffer shared-headroom-pool over-subscribe-ratio 1')
        duthost.shell('config buffer shared-headroom-pool size 0')
        check_pool_size(duthost,
                        pool_oid,
                        pool_size=original_pool_size,
        ...
        duthost.shell('config buffer shared-headroom-pool over-subscribe-ratio {}'.format(
            original_over_subscribe_ratio), module_ignore_errors=True)
        duthost.shell('config buffer shared-headroom-pool size {}'.format(
            original_configured_shp_size), module_ignore_errors=True)
        duthost.shell('config interface cable-length {} {}'.format(port_to_test,
                      original_cable_len), module_ignore_errors=True)
        ensure_pool_size(duthost, 60, pool_size_before_shp,
...
def test_lossless_pg(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test, pg_to_test,      # noqa: F811
    ...
    set_command = 'config interface buffer priority-group lossless set {} {} '.format(
        port_to_test, pg_to_test)
    add_command = 'config interface buffer priority-group lossless add {} {} '.format(
        port_to_test, pg_to_test)
    if pg_to_test == '3-4':
    ...
    try:
        ...
        if not param:
            pytest.skip(
                'Lossless pg test skipped due to no parameters for "headroom-override" command provided')
        else:
            cli_str, new_size, new_xoff = _parse_buffer_profile_params(
                param, "add", "headroom-override")
        ...
        # Create profiles
        logging.info(
            '[Preparing]: Create static buffer profile for headroom override')
        duthost.shell(cli_str)
        headroom_override_profile_oid, pool_oid = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, "headroom-override", None, None, port_to_test)
        ...
        if not param:
            pytest.skip(
                'Lossless pg test skipped due to no parameters for "non-default-dynamic_th" command provided')
        else:
            cli_str, new_size, new_xoff = _parse_buffer_profile_params(
                param, "add", "non-default-dynamic_th")

        logging.info(
            '[Preparing]: Create static buffer profile for non default dynamic_th')
        duthost.shell(cli_str)

        # Update cable length to 15m
        logging.info('[Preparing]: Update cable length')
        duthost.shell(
            'config interface cable-length {} 15m'.format(port_to_test))
        ...
        logging.info(
            '[Testcase: headroom override => dynamically calculated headroom with non-default dynamic_th]')
        duthost.shell(set_command + 'non-default-dynamic_th')
        expected_nondef_profile = make_expected_profile_name(
            original_speed, '15m', other_factors=['th2'])
        check_pg_profile(duthost, buffer_pg, expected_nondef_profile)
        # A new profile should be created in ASIC DB
        profile_oid, _ = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, expected_nondef_profile, None, pool_oid, port_to_test)

        # Update it to dynamic PG
        logging.info(
            '[Testcase: dynamically calculated headroom with non-default dynamic_th => dynamic headroom]')
        duthost.shell(set_command)
        check_pg_profile(duthost, buffer_pg, expected_profile)
        check_lossless_profile_removed(
            duthost, expected_nondef_profile, profile_oid)

        # Update it to non-default dynamic_th
        logging.info(
            '[Testcase: dynamic headroom => dynamically calculated headroom with non-default dynamic_th]')
        duthost.shell(set_command + 'non-default-dynamic_th')
        check_pg_profile(duthost, buffer_pg, expected_nondef_profile)
        # A new profile should be created in ASIC DB
        profile_oid, _ = check_buffer_profile_details(
        ...
        logging.info(
            '[Testcase: dynamically calculated headroom with non-default dynamic_th => headroom override]')
        ...
        check_lossless_profile_removed(
            duthost, 'headroom-override', headroom_override_profile_oid)
        ...
        if pg_to_test == '3-4':
            ...
            duthost.shell('config interface buffer priority-group lossless remove {} {} '.format(
                port_to_test, pg_to_test), module_ignore_errors=True)
        duthost.shell('config interface cable-length {} {}'.format(port_to_test,
                      original_cable_len), module_ignore_errors=True)
        duthost.shell(
            'config buffer profile remove headroom-override', module_ignore_errors=True)
        duthost.shell(
            'config buffer profile remove non-default-dynamic_th', module_ignore_errors=True)
        ensure_pool_size(duthost, 60, original_pool_size,
                         original_shp_size, None)
...
def test_port_admin_down(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test,      # noqa: F811
    ...
    def _check_buffer_object_aligns_between_appldb_configdb(port_to_test, key, profile_field_name):
        ...
        if objects_in_configdb:
            for object_in_configdb in objects_in_configdb:
                ...
                # Convert queue id
                object_in_app_db = _convert_ref_from_configdb_to_appldb(
                    object_in_configdb)
                profile_in_appl_db = duthost.shell(
                    'redis-cli hget "{}" {}'.format(object_in_app_db, profile_field_name))['stdout']
                pytest_assert(profile_in_appl_db == expected_profile_in_appldb,
                              "Buffer object {} contains {} which isn't expected ({})"
                              .format(key, profile_in_appl_db, expected_profile_in_appldb))
    ...
    def _check_buffer_object_aligns_with_expected_ones(port_to_test, table, expected_objects):
        ...
        """
        objects_in_appl_db = duthost.shell(
            'redis-cli keys "{}:{}:*"'.format(table, port_to_test))['stdout'].split()
        if expected_objects:
            expected_object_keys = ['{}:{}:{}'.format(
                table, port_to_test, objectid) for objectid in list(expected_objects.keys())]
            pytest_assert(set(expected_object_keys) == set(objects_in_appl_db),
                          "Objects in {} on admin-down port is {} but should be {}"
                          .format(table, objects_in_appl_db, expected_object_keys))
            for objectid, expected_profile in list(expected_objects.items()):
                profile = duthost.shell(
                    'redis-cli hget {}:{}:{} profile'.format(table, port_to_test, objectid))['stdout']
                pytest_assert(profile == expected_profile,
                              "Profile in {}:{}:{} should be {} but got {}"
                              .format(table, port_to_test, objectid, expected_objects[objectid], profile))
        else:
            pytest_assert(not objects_in_appl_db,
                          "There shouldn't be any object in {} on an administratively down port but we got {}"
                          .format(table, objects_in_appl_db))
    ...
    def _check_buffer_object_list_aligns_with_expected_ones(port_to_test, table, expected_objects):
        ...
        Args:
            ...
            expected_objects: The expected buffer items of BUFFER_PG or BUFFER_QUEUE when the port is admin down.
                              They are predefined parameters and loaded at the beginning of the test.
        """
        object_list_in_appl_db = duthost.shell(
            'redis-cli hget "{}:{}" profile_list'.format(table, port_to_test))['stdout'].split(',')
        if expected_objects:
            pytest_assert(set(expected_objects) == set(object_list_in_appl_db),
                          "Profile in {}:{} should be {} but got {}"
                          .format(table, port_to_test, expected_objects, object_list_in_appl_db))
        else:
    ...
    if not param:
        pytest.skip(
            'Shutdown port test skipped due to no headroom override parameters defined')
    ...
    if not extra_overhead:
        ...
        if not extra_overhead:
            pytest.skip(
                'Shutdown port test skipped due to no lossy pg size defined')
    ...
    dynamic_th_value = '2'
    duthost.shell('config buffer profile add {} --dynamic_th {}'.format(
        non_default_dynamic_th_profile, dynamic_th_value))

    # Create a headroom override profile
    headroom_override_profile = 'test-profile-headroom-override'
    duthost.shell('config buffer profile add {} --xon {} --xoff {}'.format(
        headroom_override_profile, param['xon'], param['xoff']))

    _, pool_oid = check_buffer_profile_details(
        duthost, initial_asic_db_profiles, headroom_override_profile, None, None, port_to_test)
    ...
    """
        Each item is a tuple consisting of:
         ...
         - Whether we need to check whether the previous expected_profile has been removed
           after port is administratively down
    """
    scenarios = [
        ('Remove the generic PG when port is administratively down',
         'config interface buffer priority-group lossless remove {} 3-4'.format(
             port_to_test),
         None,
         False),
        ('Add a PG with non default dynamic_th when port is administratively down',
         'config interface buffer priority-group lossless add {} 3-4 {}'.format(
             port_to_test, non_default_dynamic_th_profile),
         make_expected_profile_name(original_speed, original_cable_len, other_factors=[
                                    'th{}'.format(dynamic_th_value)]),
         False),
        ...
        ('Add a PG with headroom override profile when port is administratively down',
         'config interface buffer priority-group lossless add {} 3-4 {}'.format(
             port_to_test, headroom_override_profile),
    ...
    try:
        for scenario in scenarios:
            ...
            expected_pgs = TESTPARAM_ADMIN_DOWN.get('BUFFER_PG_TABLE')
            _check_buffer_object_aligns_with_expected_ones(
                port_to_test, 'BUFFER_PG_TABLE', expected_pgs)

            # Make sure the zero profiles have been applied on queues on the port
            logging.info(
                'Check whether all queues are configured as zero profile or removed from port {}'
                .format(port_to_test))
            expected_queues = TESTPARAM_ADMIN_DOWN.get('BUFFER_QUEUE_TABLE')
            _check_buffer_object_aligns_with_expected_ones(
                port_to_test, 'BUFFER_QUEUE_TABLE', expected_queues)

            # Make sure the zero profiles have been applied on ingress buffer profile list on the port
            logging.info(
                'Check whether ingress profile list is configured as zero profile or removed from port {}'
                .format(port_to_test))
            expected_ingress_profile_list = TESTPARAM_ADMIN_DOWN.get(
                'BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE')
            _check_buffer_object_list_aligns_with_expected_ones(
                port_to_test, 'BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE', expected_ingress_profile_list)

            # Make sure the zero profiles have been applied on egress buffer profile list on the port
            logging.info(
                'Check whether egress profile list is configured as zero profile or removed from port {}'
                .format(port_to_test))
            expected_egress_profile_list = TESTPARAM_ADMIN_DOWN.get(
                'BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE')
            _check_buffer_object_list_aligns_with_expected_ones(
                port_to_test, 'BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE', expected_egress_profile_list)
            ...
            if need_remove_previous_profile:
                logging.info('Check whether profile {} has been removed after port being administratively down'.format(
                    previous_profile))
            ...
            if expected_profile_in_appldb:
                logging.info('Check whether profile in PG is as expected({})'.format(
                    expected_profile_in_appldb))
                check_pg_profile(
                ...
                pgs_in_appl_db = duthost.shell(
                    'redis-cli keys "BUFFER_PG_TABLE:{}:3-4"'.format(port_to_test))['stdout']
                pytest_assert(
                    not pgs_in_appl_db, "There shouldn't be PGs 3-4 but we got {}".format(pgs_in_appl_db))

            # Check whether the queues have been applied correctly
            logging.info(
                'Check whether queues are readded to port {}'.format(port_to_test))
            _check_buffer_object_aligns_between_appldb_configdb(
                port_to_test, "BUFFER_QUEUE|{}|*".format(port_to_test), 'profile')

            # Check whether the ingress profile list have been applied correctly
            logging.info(
                'Check whether ingress profile list are readded to port {}'.format(port_to_test))
            _check_buffer_object_aligns_between_appldb_configdb(
                port_to_test, "BUFFER_PORT_INGRESS_PROFILE_LIST|{}".format(port_to_test), 'profile_list')

            # Check whether the egress profile list have been applied correctly
            logging.info(
                'Check whether egress profile list are readded to port {}'.format(port_to_test))
            _check_buffer_object_aligns_between_appldb_configdb(
                port_to_test, "BUFFER_PORT_EGRESS_PROFILE_LIST|{}".format(port_to_test), 'profile_list')
        ...
        duthost.shell('config interface startup {}'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config interface buffer priority-group lossless set {} 3-4'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config interface buffer priority-group lossless add {} 3-4'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config buffer profile remove {}'.format(
            non_default_dynamic_th_profile), module_ignore_errors=True)
        duthost.shell('config buffer profile remove {}'.format(
            headroom_override_profile), module_ignore_errors=True)
        ensure_pool_size(duthost, 60, original_pool_size,
                         original_shp_size, None)
...
def test_port_auto_neg(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test,      # noqa: F811
    ...
    The flow of the test case:
        ...
        3. Enable the port auto negotiation and then configure the advertised speed list and then disable it
           - The maximum supported speed should be taken into account
             for buffer calculation after port auto negotiation enabled
    ...
    Note:
        ...
        - The buffer pool size depends on the buffer priority-groups and
          profiles but not directly on the effective speed.
          As buffer pool size has been verified in other test cases and
          checking it will consume more time, we don't repeat it here.
    """
    ...
    initial_asic_db_profiles = fetch_initial_asic_db(duthost)
    expected_profile = make_expected_profile_name(
        speed_before_test, cable_length_to_test)
    try:
        # Preparing: configure the speed to one which is not the maximum speed and the cable length to 15m
        # This is to enforce there is a new buffer profile created
        duthost.shell('config interface speed {} {}'.format(
            port_to_test, speed_before_test))
        duthost.shell(
            'config interface cable-length {} {}'.format(port_to_test, cable_length_to_test))
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile)
        new_profile_id, pool_id = check_buffer_profile_details(
            duthost, initial_asic_db_profiles, expected_profile, None, None, port_to_test)
        ...
        duthost.shell(
            'config interface autoneg {} enabled'.format(port_to_test))
        # Check whether the maximum supported speed is used for creating lossless profile
        expected_profile = make_expected_profile_name(
            max_supported_speed, cable_length_to_test)
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile)
        check_buffer_profile_details(
            duthost, initial_asic_db_profiles, expected_profile, new_profile_id, pool_id, port_to_test)

        # Configure advertised speeds
        logging.info('Update advertised speeds to {}'.format(
            advertised_speeds_to_test))
        duthost.shell(
            'config interface advertised-speeds {} {}'.format(port_to_test, advertised_speeds_to_test))
        # Check whether the maximum advertised speed is used for creating lossless profile
        expected_profile = make_expected_profile_name(
            max_advertised_speed, cable_length_to_test)
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile)
        check_buffer_profile_details(
            duthost, initial_asic_db_profiles, expected_profile, new_profile_id, pool_id, port_to_test)
        ...
        duthost.shell(
            'config interface autoneg {} disabled'.format(port_to_test))
        expected_profile = make_expected_profile_name(
            speed_before_test, cable_length_to_test)
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile)
        ...
        logging.info(
            'Reenable port auto negotiation with advertised speeds configured')
        duthost.shell(
            'config interface autoneg {} enabled'.format(port_to_test))
        # Check whether the maximum advertised speed is used for creating lossless profile
        expected_profile = make_expected_profile_name(
        ...
        duthost.shell(
            'config interface advertised-speeds {} all'.format(port_to_test))
        expected_profile = make_expected_profile_name(
            max_supported_speed, cable_length_to_test)
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile)
        ...
        # Clean up
        duthost.shell('config interface buffer priority-group lossless remove {} 6'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config interface cable-length {} {}'.format(port_to_test,
                      original_cable_length), module_ignore_errors=True)
        duthost.shell('config interface speed {} {}'.format(
            port_to_test, original_speed), module_ignore_errors=True)
        duthost.shell(
            'config interface advertised-speeds {} all'.format(port_to_test), module_ignore_errors=True)
        duthost.shell('config interface autoneg {} disabled'.format(
            port_to_test), module_ignore_errors=True)
        ensure_pool_size(duthost, 60, original_pool_size,
                         original_shp_size, None)
...
def test_exceeding_headroom(duthosts, rand_one_dut_hostname,
    ...
    try:
        ...
        while True:
            ...
            expected_profile = make_expected_profile_name(
                original_speed, '{}m'.format(cable_length))
            profile_applied = check_pg_profile(
                duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), expected_profile, False)
            if not profile_applied:
                break
            logging.info(
                'Cable length {} has been applied successfully'.format(cable_length))
            if cable_length > 10000:
                pytest.skip(
                    "Not able to find the maximum headroom of port {} after cable length has been increased to 10km, "
                    "skip the test".format(port_to_test))
        ...
        while True:
            ...
            if profile_applied:
                cable_length_lower = cable_length
                logging.info('Cable length {} has been applied successfully, moving forward. Range now is [{}, {})'
                             .format(cable_length, cable_length_lower, cable_length_upper))
            else:
                cable_length_upper = cable_length
                logging.info('Cable length {} has not been applied, moving backward. Range now is [{}, {})'
                             .format(cable_length, cable_length_lower, cable_length_upper))
        ...
        logging.info('Got maximum cable length {}'.format(
        ...
        maximum_profile = _compose_dict_from_cli(duthost.shell(
            'redis-cli hgetall BUFFER_PROFILE_TABLE:{}'.format(maximum_profile_name))['stdout'].split())

        # Config the cable length to the longest acceptable value and check the profile
        logging.info(
            '[Config the cable length to the longest acceptable value on the port]')
        duthost.shell(
            'config interface cable-length {} {}m'.format(port_to_test, maximum_cable_length))
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), maximum_profile_name)

        # 2. Add extra PGs to a port, which causes the accumulative headroom exceed the limit
        logging.info('Add another PG and make sure the system isn\'t broken')
        duthost.shell(
            'config interface buffer priority-group lossless add {} {}'.format(port_to_test, '5-7'))
        profile_applied = check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:5-7'.format(port_to_test), maximum_profile_name, False)
        pytest_assert(not profile_applied,
                      "Profile {} applied on {}:5-7, which makes the accumulative headroom exceed the limit"
                      .format(maximum_profile_name, port_to_test))
        ...
        duthost.shell(
            'config interface buffer priority-group lossless remove {} {}'.format(port_to_test, '5-7'))
        duthost.shell(
            'config interface cable-length {} {}'.format(port_to_test, original_cable_len))
        ...
        duthost.shell('config interface buffer priority-group lossless set {} {} {}'.format(
            port_to_test, '3-4', 'test-headroom'))

        logging.info('Verify the profile is applied')
        check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:3-4'.format(port_to_test), 'test-headroom')

        # Apply the profile on other PGs, which make the accumulative headroom exceed the limit
        duthost.shell('config interface buffer priority-group lossless add {} {} {}'.format(
            port_to_test, '5-7', 'test-headroom'))
        # Make sure the profile hasn't been applied
        profile_applied = check_pg_profile(
            duthost, 'BUFFER_PG_TABLE:{}:5-7'.format(port_to_test), 'test-headroom', False)
        pytest_assert(not profile_applied,
                      "Profile {} applied on {}:5-7, which makes the accumulative headroom exceed the limit"
                      .format(maximum_profile_name, port_to_test))
        ...
        duthost.shell('config interface cable-length {} {}'.format(port_to_test,
                      original_cable_len), module_ignore_errors=True)
        duthost.shell('config interface buffer priority-group lossless remove {} 5-7'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config interface buffer priority-group lossless set {} 3-4'.format(
            port_to_test), module_ignore_errors=True)
        duthost.shell('config buffer profile remove test-headroom',
...
def test_buffer_deployment(duthosts, rand_one_dut_hostname, conn_graph_facts, tbinfo, dualtor_ports,      # noqa: F811
    ...
    def _check_buffer_item_in_asic_db(dut_db_info, port, buffer_item, name_map,
                                      buffer_profile_oid, asic_key_name, should_have_profile, use_assert):
    ...
    def _check_port_buffer_info_and_get_profile_oid(dut_db_info, table, ids, port, expected_profile, use_assert=True):
        ...
        """
        profile_in_db = dut_db_info.get_profile_name_from_appl_db(
            table, port, ids)
        ...
        if expected_profile:
            ...
            if buffer_name_map:
                buffer_profile_oid = None
                for item in id_list:
                    logging.info("Checking {}:{}:{} in ASIC_DB, expected_profile:{}".format(
                        table, port, item, expected_profile))
                    buffer_profile_oid, success = _check_buffer_item_in_asic_db(
                        dut_db_info, port, item, buffer_name_map, buffer_profile_oid, sai_field, True, use_assert)
                    if not success:
                        return None, False
        else:
            if not _check_condition(not profile_in_db, "{}:{}:{} configured on admin down port"
                                    .format(table, port, ids), use_assert):
                return None, False
            if buffer_name_map:
                for item in id_list:
                    logging.info(
                        "Checking {}:{}:{} in ASIC_DB".format(table, port, item))
                    buffer_profile_oid, success = _check_buffer_item_in_asic_db(
                        dut_db_info, port, item, buffer_name_map, None, sai_field, False, use_assert)
    ...
    def _check_port_buffer_info_and_return(dut_db_info, table, ids, port, expected_profile):
        ...
        _, result = _check_port_buffer_info_and_get_profile_oid(
            dut_db_info, table, ids, port, expected_profile, False)
        return result
    ...
    buffer_table_up = {
        KEY_2_LOSSLESS_QUEUE: [('BUFFER_PG_TABLE', '0', '[BUFFER_PROFILE_TABLE:ingress_lossy_profile]'),
                               ...
                               ('BUFFER_QUEUE_TABLE', '3-4',
                                '[BUFFER_PROFILE_TABLE:egress_lossless_profile]'),
                               ('BUFFER_QUEUE_TABLE', '5-6',
                                '[BUFFER_PROFILE_TABLE:q_lossy_profile]'),
                               (None, None, None)
                               ],
        KEY_4_LOSSLESS_QUEUE: [('BUFFER_PG_TABLE', '0', '[BUFFER_PROFILE_TABLE:ingress_lossy_profile]'),
                               ('BUFFER_QUEUE_TABLE', '0-1',
                                '[BUFFER_PROFILE_TABLE:q_lossy_profile]'),
                               ('BUFFER_QUEUE_TABLE', '2-4',
                                '[BUFFER_PROFILE_TABLE:egress_lossless_profile]'),
                               ('BUFFER_QUEUE_TABLE', '5',
                                '[BUFFER_PROFILE_TABLE:q_lossy_profile]'),
                               ('BUFFER_QUEUE_TABLE', '6',
                                '[BUFFER_PROFILE_TABLE:egress_lossless_profile]'),
                               ('BUFFER_QUEUE_TABLE', '7',
                                '[BUFFER_PROFILE_TABLE:q_lossy_profile]'),
                               (None, None, None)
                               ]
    }
    if is_tunnel_qos_remap_enabled(duthost):
        buffer_table_up[KEY_2_LOSSLESS_QUEUE][3] = (
            'BUFFER_QUEUE_TABLE', '5-7', '[BUFFER_PROFILE_TABLE:q_lossy_profile]')

    if not is_mellanox_device(duthost):
        buffer_table_up[KEY_2_LOSSLESS_QUEUE][1] = (
            'BUFFER_QUEUE_TABLE', '0-2', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')
        if is_tunnel_qos_remap_enabled(duthost):
            buffer_table_up[KEY_2_LOSSLESS_QUEUE][3] = (
                'BUFFER_QUEUE_TABLE', '5-7', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')
        else:
            buffer_table_up[KEY_2_LOSSLESS_QUEUE][3] = (
                'BUFFER_QUEUE_TABLE', '5-6', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')

        buffer_table_up[KEY_4_LOSSLESS_QUEUE][1] = (
            'BUFFER_QUEUE_TABLE', '0-1', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')
        buffer_table_up[KEY_4_LOSSLESS_QUEUE][3] = (
            'BUFFER_QUEUE_TABLE', '5', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')
        buffer_table_up[KEY_4_LOSSLESS_QUEUE][5] = (
            'BUFFER_QUEUE_TABLE', '7', '[BUFFER_PROFILE_TABLE:egress_lossy_profile]')
    ...
    buffer_table_down = {
        ...
        # The admin_down ports can not be dualtor_ports. Hence there is no 4_lossless_queue profile
        KEY_4_LOSSLESS_QUEUE: [(None, None, None)]
    }

    if is_tunnel_qos_remap_enabled(duthost):
        buffer_table_down[KEY_2_LOSSLESS_QUEUE][3] = (
            'BUFFER_QUEUE_TABLE', '5-7', '[BUFFER_PROFILE_TABLE:egress_lossy_zero_profile]')
    ...
    for port in configdb_ports:
        ...
        for table, ids, expected_profile in buffer_items_to_check:
            logging.info("Checking buffer item {}:{}:{}, expected_profile:{}".format(
                table, port, ids, expected_profile))
            ...
            if is_qos_db_reference_with_table:
                ...
                expected_profile_key = "BUFFER_PROFILE_TABLE:{}".format(
                    expected_profile)

            if expected_profile not in profiles_checked:
                profile_info = dut_db_info.get_profile_info_from_appl_db(
                ...
                if is_ingress_lossless and not BUFFER_MODEL_DYNAMIC:
                    std_profiles_for_speed = DEFAULT_LOSSLESS_HEADROOM_DATA.get(
                        speed)
                    if std_profiles_for_speed:
                        std_profile = std_profiles_for_speed.get(cable_length)
                        if std_profile:
                            ...
                            # We can check whether the headroom data is correct
                            pytest_assert(
                                profile_info['xon'] == std_profile['xon'] and
                                profile_info['xoff'] == std_profile['xoff']
                                and (profile_info['size'] == std_profile['size']
                                     or DEFAULT_SHARED_HEADROOM_POOL_ENABLED),
    ...
    if not BUFFER_MODEL_DYNAMIC:
        ...
        def _profile_name(duthost, port, pg_id_name):
            if is_mellanox_device(duthost):
                ...
                profile_name = duthost.shell(
                    'redis-cli hget "BUFFER_PG_TABLE:{}:{}" profile'.format(port, pg_id_name))['stdout']
        ...
        try:
            ...
            duthost.shell(
                'config interface shutdown {}'.format(port_to_shutdown))
            for pg_id_name in pg_id_names:
                wait_until(60, 5, 0, _check_port_buffer_info_and_return, dut_db_info, 'BUFFER_PG_TABLE',
                           pg_id_name, port_to_shutdown, _profile_name(duthost, port, pg_id_name))
            ...
            duthost.shell(
                'config interface startup {}'.format(port_to_shutdown))
            for pg_id_name in pg_id_names:
                wait_until(60, 5, 0, _check_port_buffer_info_and_return, dut_db_info, 'BUFFER_PG_TABLE',
                           pg_id_name, port_to_shutdown, _profile_name(duthost, port, pg_id_name))
        finally:
            duthost.shell('config interface startup {}'.format(
                port_to_shutdown), module_ignore_errors=True)
...
def mellanox_calculate_headroom_data(duthost, port_to_test):
    ...
    It intends to calculate the headroom size based on the input port attributes
    (speed, cable_length, number of lanes..., etc)
    This algorithm is the same as the implementation in
    https://github.com/sonic-net/sonic-swss/blob/master/cfgmgr/buffer_headroom_mellanox.lua
    """
    global ASIC_TABLE_KEYS_LOADED
    global CELL_SIZE
    global PIPELINE_LATENCY
    ...
    if port_info.get('autoneg') == 'on':
        ...
        if adv_speeds and adv_speeds != 'all':
            ...
            available_speeds = duthost.shell(
                'redis-cli -n 6 hget "PORT_TABLE|{}" "supported_speeds"'.format(port_to_test))['stdout']
    ...
    if port_speed_raw:
        ...
        logging.error(
            "failed to get speed from config db for port {}".format(port_to_test))
        return False, None
    ...
    if port_speed in list(pause_quanta_per_speed_dict.keys()):
        ...
        peer_response_time_keys = duthost.shell(
            'redis-cli -n 6 keys ASIC_TABLE*')['stdout']
        peer_response_time = float(duthost.shell(
            'redis-cli -n 6 hget "{}" "peer_response_time"'.format(peer_response_time_keys))['stdout'])
    ...
    if cable_length_raw and cable_length_raw.endswith('m'):
        ...
        logging.error(
            "failed to get a valid cable length from config db for port {}".format(port_to_test))
        return False, None

    logging.info('port_speed = {}, port_mtu = {}, cable_length = {}'.format(
        port_speed, port_mtu, cable_length))
    ...
    logging.debug('gearbox_delay = {}, bytes_on_gearbox = {}'.format(
        gearbox_delay, bytes_on_gearbox))
