...
<<<<<<< HEAD
=======
        # Validate all expected queues are present and programmed (non-zero).
        missing = EXPECTED_COS_QUEUES - set(before_pps.keys())
        assert not missing, \
            "CPU queue shaper missing for cos queues {} before reboot. Got: {}".format(missing, before_pps)
>>>>>>> 921aacd21 (NOS-10066: [cpu_shaper] Validate shaper persistence, not a hardcoded PPS (fix TH6/Sea Eagle) (#1989))
...
=======
        ...
        # Verify shaper config survived reboot unchanged (the actual invariant).
