...
def test_dialout_stream(dialout_setup, telemetry_client_config, start_collector):
    ...
    # Payload check: the subscription asks for COUNTERS/Ethernet*, so
    # updates must arrive at COUNTERS/<port> for real DUT ports, each
    # carrying a non-empty counters payload.
    ...
    for record in collector.records():
        for path, payload in record.get("updates", []):
            if path.startswith("COUNTERS/"):
                port = path.split("/", 1)[1]
                pytest_assert(port in ports,
                              "counters update for a port the DUT does not have: %s" % path)
