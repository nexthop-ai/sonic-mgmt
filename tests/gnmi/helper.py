...
def gnmi_subscribe_polling_py(duthost, ptfhost, path_list, target, polling_interval, update_count,
    ...
    Mirrors the telemetry client's POLL request shape: the DB is given via the
    target (-xt <target>[/<namespace>]) with table-relative xpaths, not via a
    sonic-db origin with the DB in the path.
